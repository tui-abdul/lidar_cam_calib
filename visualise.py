...
class camera_publisher(Node):
    ...
    def __init__(self):
        ...
        #self.rvec = np.array([  1.21624732, -1.22809485 , 1.14942506],dtype=np.float32).reshape((3,1)) #data from car best so far
        #self.tvec = np.array([0.0590897,-0.12270744,0.85991654],dtype=np.float32).reshape((3,1)) #data from car best so far
        self.tvec = np.array([ 0.11883379, -0.00222689 , -0.08074747],dtype=np.float64).reshape((3,1)) #data from car
        self.rvec = np.array([ 1.21836062,-1.19281464, 1.24768474],dtype=np.float64).reshape((3,1)) #data from car
